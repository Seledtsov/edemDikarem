...
v_location=new String(window.location);
...
//    ['undo','redo','separator'],
    ...
    ['HorizontalRule', 'insertmarquee', 'Createlink','InsertImage','InsertTable','htmlmode','separator'],

  ['custom1'],['custom2'], ['custom4'], ['custom3'], //'custom2','custom3','separator'],
...
//alert('1 '+v_location);
//test_mzd=v_location.search(/mzd/i);
//alert('test_mzd='+test_mzd);
v_location=new String(window.location);
//alert(v_location);

var ar_location=Array();

var ar_domen=Array();

ar_location=v_location.split("\/");

//alert(ar_location[2]);


//alert('css_site='+css_site);
//alert(this.stylesheet);
if(typeof(css_site)!="undefined")
{
this.stylesheet=css_site;
//если стиль - без указания хоста
if( (this.stylesheet.search(/http/i))==-1 )
     {
     this.stylesheet="http://"+ar_location[2]+this.stylesheet;
     }
}
//alert(this.stylesheet);
stylesfile=this.stylesheet;
if(typeof(special_style)!="undefined")
{
this.fontstyles = [     // make sure these exist in the header of page the content is being display as well in or they won't work!
    { name: "special",     className: "special",  classStyle: special_style }
];
}
...
this.btnList = {
    ...
    "bold":           ['Bold',                 'Полужирный',         'editor_action(this.id)',  'ed_format_bold.gif'],
    ...
    "justifyleft":    ['JustifyLeft',          'По левому краю',       'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        'По центру',     'editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         'По правому краю',      'editor_action(this.id)',  'ed_align_right.gif'],
    "justifyfull":   ['JustifyFull',         'По краям',      'editor_action(this.id)',  'justifyfull.gif'],
    "copy":   ['Copy',         'Копировать',      'editor_action(this.id)',  'ico_copy.gif'],
    "paste":   ['Paste',         'Вставить',      'editor_action(this.id)',  'ico_paste.gif'],
    "cut":   ['Cut',         'Вырезать',      'editor_action(this.id)',  'ico_cut.gif'],
    "orderedlist":    ['InsertOrderedList',    'Нумерация',       'editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',  'Маркеры',      'editor_action(this.id)',  'ed_list_bullet.gif'],
    ...
    "forecolor":      ['ForeColor',            'Цвет шрифта',         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',            'Выделение цветом',   'editor_action(this.id)',  'ed_color_bg.gif'],
    "undo":   ['Undo',         'Отменить',      'editor_action(this.id)',  'ico_undo.gif'],
    "redo":   ['Redo',         'Вернуть',      'editor_action(this.id)',  'ico_redo.gif'],
    "horizontalrule": ['InsertHorizontalRule', 'Горизонтальная черта',    'editor_action(this.id)',  'ed_hr.gif'],
    "insertmarquee": ['InsertMarquee', 'Бегущая строка',    'editor_action(this.id)',  'ed_marq.gif'],
    "createlink":     ['CreateLink',           'Добавить гиперссылку',    'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          'Добавить картинку',       'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          'Добавить таблицу',       'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'Просмотр HTML-кода',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          'Большое окно редактирования',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',                'О редакторе',  'editor_about(\''+objname+'\')',  'ed_about.gif'],

    // Add custom buttons here:

    "custom1":           ['custom1',         'Отмена форматирования документа',  'editor_action(this.id)',  'ed_custom.gif'],
    "custom2":           ['custom2',         'Стандартный шрифт и размер текста',  'editor_action(this.id)',  'ed_custom1.gif'],
    "custom3":           ['custom3',         'Обновить стили',  'editor_action(this.id)',  'ed_custom3.gif'],
    "custom4":           ['custom4',         'Удалить PRE',  'editor_action(this.id)',  'ed_nopre.gif'],
   // end: custom buttons
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      if (btnName == "fontname") {
        ...
        toolbar += '<select id="_' +objname+ '_FontName" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; width:100px; font-size: 12px;">';
      ...
      // fontsize
      if (btnName == "fontsize") {
        toolbar += '<select id="_' +objname+ '_FontSize" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; width:60px; font-size: 12px;">';
      ...
      if (btnName == "fontstyle") {
        toolbar += '<select id="_' +objname+ '_FontStyle" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 0; width:100px; font-size: 12px;">';
  ...
  if (config.plaintextInput) {     // replace nextlines with breaks
    ...
    test_br=contents.match(/<br>/i);
    test_table=contents.match(/<table/i);
    test_p=contents.match(/<p/i);
    //alert('test_br='+test_br);
    //alert(v_location);
    if(test_br==null && test_table==null && test_p==null) {
     contents = contents.replace(/\r\n/g, '</p><p>');
     contents = contents.replace(/\n/g, '</p><p>');
     contents = contents.replace(/\r/g, '</p><p>');
     }



    var_len=contents.length;

    if(var_len>0){

     var_test_m=contents.charCodeAt(0);

     if(var_test_m!=160 && (objname!='announce_new')) {contents=var_add+contents;}

     //alert('var_test_m=' + var_test_m + '\r\n');
...
if(confirm("Вы действительно хотите отменить все форматирование?")){
    deleteformatHTML(objname);
    }
  }
  // Custom2
  else if (cmdID == 'custom2') {
if(confirm("Вы действительно хотите использовать стандартный шрифт?")){
    deletefont(objname);
    }
  }
  // Custom3
  ...
  //global stylesfile;
  //alert(stylesfile);
  css_window=window.open(stylesfile, 'css', 'width=1, height=1');
  css_window.close();
  }
  // Custom4
  else if (cmdID == 'custom4') {
    //alert("Hello, I am custom button 1!");
if(confirm("Вы действительно хотите удалить тэги PRE?")){
    deletePRE(objname);
    }
  }
 ...
 // FontSize
    ...
    //editdoc.execCommand('FontSize',0, val);
       c_size(objname, val);
	  }
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "popups/insert_image.html", editdoc, "resizable: yes; help: no; status: no; scroll: no;");
...
function editor_filterOutput(objname) {
  ...
  //if (contents.toLowerCase() == '<br>') { contents = ""; }
...
function editor_insertHTML(objname, str1,str2, reqSel) {
  ...
  if (editor_obj == null) { return alert("Unable1 to insert HTML.  Invalid object name '" +objname+ "'."); }
...
//alert('sHtml='+sHtml);
    ...
    if (sRange.length) { return alert("Unable2 to insert HTML.  Try highlighting content instead of selecting it."); }
    ...
    window.onerror = function() { alert("Unable3 to insert HTML for current selection."); return true; } // partial table selections cause errors
    ...
    if (sHtml.length) {                                 // if content selected
      ...
      if (reqSel) { return alert("Unable4 to insert HTML.  You must select something first."); }
  ...
  // insertHTML for plaintext textarea
    ...
    if (sText.length) {                                 // if content selected
      ...
      if (reqSel) { return alert("Unable5 to insert HTML.  You must select something first."); }
  ...
  else { alert("Unable6 to insert HTML.  Unknown object tag type '" +tagname+ "'."); }
...
/* ---------------------------------------------------------------------- *\
  ...
  Function    : deleteformatHTML

\* ---------------------------------------------------------------------- */
function deleteformatHTML(objname) {
  ...
  if (document.all[objname] && editor_obj == null) {
    document.all[objname].focus();
    document.all[objname].value = document.all[objname].value + "test";
    return;
  }

  // error checking
  if (editor_obj == null) { return alert("Unable7 to insert HTML.  Invalid object name '" +objname+ "'."); }
  editor_focus(editor_obj);
  var tagname = editor_obj.tagName.toLowerCase();
  var sRange;
...
//    alert('str1='+str1);
   ...
   // check for control ranges
    if (sRange.length) { return alert("Unable8 to insert HTML.  Try highlighting content instead of selecting it."); }

    // insert HTML
    var oldHandler = window.onerror;
    window.onerror = function() { alert("Unable9 to insert HTML for current selection."); return true; } // partial table selections cause errors

    sRange.pasteHTML("");
    sRange.collapse("start");
    sRange.expand("textedit");
   ...
   if(sHtml.match('EM>')){
    ...
    if(sHtml.match('STRONG>')){
        str1='</STRONG>'+str1+'<STRONG>';
        }

    if (sHtml.length) {                                 // if content selected
       sRange.pasteHTML(str1);              // overwrite
    } else {                                            // if insertion point only
...
function deletefont(objname, mode) {
  var config     = document.all[objname].config;
  var editor_obj = document.all["_" +objname + "_editor"];
  if (document.readyState != 'complete') {
    setTimeout(function() { editor_setmode(objname, mode) }, 25);
  ...
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    var editdoc = editor_obj.contentWindow.document;
    var contents = editdoc.body.createTextRange().htmlText;
    //alert("1 contents="+contents);

    contents = contents.replace(/<\/?H[1-7]?[^>]*>/gi, '');
    contents = contents.replace(/<\/*FONT[^>]*>/gi, '');
    contents = contents.replace(/class=[^>]*/gi, '');
    contents = contents.replace(/style="[^\"]*"/gi, '');
    contents = contents.replace(/<\/?o:p>/gi, '');
    contents = contents.replace(/<\/?v:[^>]*>/gi, '');
    contents = contents.replace(/<\/?span[^>]*>/gi, '');
    contents = contents.replace(/<\/?span>/gi, '');
    contents = contents.replace(/<BR>&nbsp;&nbsp;&nbsp;&nbsp;/gi, '</P><P>');
    contents = contents.replace(/&nbsp;&nbsp;&nbsp;&nbsp;/gi, '</P><P>');
    contents='<P>'+contents;
    contents = contents.replace(/<br>/gi, '</P><P>');
    contents = contents.replace(/^<\/P>/, '');
    contents = contents.replace(/^<P><P>/, '<P>');
    contents = contents.replace(/<P><\/P>/gi, '<P>&nbsp;<\/P>');
    //contents = contents.replace(/<o:p>/gi, '');
    //alert(contents);
    //str1 = contents.replace(/style="[a-z0-9: ;\-\s]*"/gi, '');

    //contents = contents.replace(/<table/gi, '<table class="text" ');
    contents = contents.replace(/http:\/\/ism.cwp.mps.ru/gi, '');
    contents = contents.replace(/http:\/\/ism-eng.cwp.mps.ru/gi, '');
    contents = contents.replace(/http:\/\/ism.cwp.rzd.ru/gi, '');
    contents = contents.replace(/http:\/\/ism-eng.cwp.rzd.ru/gi, '');
    //contents = contents.replace(/<p>/gi, '<br>\r\n&nbsp;&nbsp;&nbsp;&nbsp;');
    //contents = contents.replace(/<\/p>/gi, '');
    contents = contents.replace(/&amp;/gi, '&');

    //alert("2 contents="+contents);
    editor_obj.outerHTML = TextEdit;
    editor_obj = document.all["_" +objname + "_editor"];
    editor_obj.value = contents;
    editor_event(objname);
...
-----------------------------------------
*/
function c_size(objname, val) 
{
alert('c_size');
//editor_insertHTML(objname, '<span style="font-size:' + val + '">', '</span>');
var str1='<span style="font-size:' + val + '">';
var str2='</span>'
  var config     = document.all[objname].config;
  var editor_obj = document.all["_" +objname + "_editor"];    // editor object
  if (str1 == null) { str1 = ''; }
...
//alert('length='+sHtml.length+'sHtml='+sHtml);

if (typeof(sHtml)!='undefined' && sHtml.length!="null" && sHtml.length){
    sHtml = sHtml.replace(/size=[0-9]*/gi, '');
    sHtml = sHtml.replace(/font-size:\s*[0-9a-z-]*/gi, '');
    sHtml = sHtml.replace(/style=""/gi, '');
} else {
alert('Не выделен текст для форматирования');
      return true;
}
    //alert('sHtml=\''+sHtml);
    // check for control ranges
    ...
    // insert HTML
    var oldHandler = window.onerror;
    //window.onerror = function() { alert("Unable3 to insert HTML for current selection."+oldHandler); return true; } // partial table selections cause errors
    if (typeof(sHtml.length)!="undefined" && sHtml.length) 
		{// if content selected
       sRange.pasteHTML(str1 +sHtml+ str2);
	  editor_obj.focus();
    } else {  // if insertion point only
      if (typeof(reqSel)!="undefined" && reqSel) { return alert("Unable4 to insert HTML.  You must select something first."); }
      alert('Не выделен текст для форматирования');
      return true;
    }
    window.onerror = oldHandler;
  ...
  // insertHTML for plaintext textarea
    ...
    var sText   = sRange.text;
    // insert HTML
    if (typeof(sText.length)!="undefined" && sText.length ) {                                 // if content selected
      if (str2) { sRange.text = str1 +sText+ str2; }  // surround
      else      { sRange.text = str1; }               // overwrite
    } else {                                            // if insertion point only
      if (reqSel) { return alert("Unable5 to insert HTML.  You must select something first."); }
...
/* ---------------------------------------------------------------------- *\
  Function    : deletePRE
\* ---------------------------------------------------------------------- */
function deletePRE(objname, mode)
{
var config     = document.all[objname].config;
  var editor_obj = document.all["_" +objname + "_editor"];
  if (document.readyState != 'complete') {
  ...
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    contents = contents.replace(/<pre[^>]*>/gi, '<P>');
    contents = contents.replace(/<\/pre>/gi, '</P>');
