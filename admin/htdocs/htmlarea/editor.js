...
document.write('.text {FONT-SIZE: 12px; FONT-FAMILY: Verdana,Arial,Times}');
...
this.version = "2.03"
if (typeof HTML_EDIT_WIDTH == "undefined" || HTML_EDIT_WIDTH==0){
this.width =  "auto";
}else{
this.width =HTML_EDIT_WIDTH;
}
//alert('HTML_EDIT_WIDTH='+HTML_EDIT_WIDTH);
this.height = "auto";
//this.bodyStyle = 'background-color: #FFFFFF; font-family: "Verdana"; font-size: x-small; ';
//this.bodyStyle = 'FONT-SIZE: 12px; FONT-FAMILY: "Verdana,Arial,Times;"';
...
//  ['strikethrough','subscript','superscript','separator'],
  ...
  ['custom1'],['custom2'],//'custom2','custom3','separator'],
    ['popupeditor','about']];
...
this.stylesheet = "http://www.mzd.ru/css/rjd.css"; // full URL to stylesheet
...
this.btnList = {
    ...
    "custom2":           ['custom2',         'Стандартный шрифт и размер текста',  'editor_action(this.id)',  'ed_custom.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  if (config.plaintextInput) {     // replace nextlines with breaks
    ...
    if(test_br==null && test_table==null && test_p==null) {
     ...
     contents = contents.replace(/\n/g, '<br>'+var_add);
     contents = contents.replace(/\r/g, '<br>'+var_add);
...
/* ---------------------------------------------------------------------- *\
  Function    : deletefontHTML
\* ---------------------------------------------------------------------- */

function deletefontHTML(objname) {
  var config= document.all[objname].config;
  var editor_obj = document.all["_" +objname + "_editor"];    // editor object
  var str1='';
 ...
 // insertHTML for wysiwyg iframe
  if (tagname == 'iframe') {
    ...
    str1 = str1.replace(/class=\S*/gi, '');
    str1 = str1.replace(/<table/gi, '<table class="text" ');
...
function deletefont(objname, mode) {
  ...
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    contents = contents.replace(/class=\S*/gi, '');
