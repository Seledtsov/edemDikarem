...
class Template {
    ...
    function Template($options)
    {
        ...
        if (is_scalar($options)) {
            ...
            echo("Template->Template() : Trying to create template without providing a template filename or an options array");
            exit;
        }
        ...
        if (!is_readable($filename)) {
            echo("Template->Template() : Template file \"".$filename."\" not found");
            exit;
        }
        // BEGIN DEBUG
    ...
    function Parse()
    {
        ...
        for ($i = 0; $i < count($chunks); $i++) {
            if (preg_match($regex2, $chunks[$i], $tag)) {
                ...
                if (!preg_match("/^[-\w\/+_\.]*$/", $name)) {
                    echo("Template::Parse() : Invalid character(s) in NAME attribute (".htmlentities($name).") for ".$which." tag, found at ".$this->options['filename'].", line ".$lineNumber);
                    exit;
                }

                // Die if we need a name and didn't get one
                if (empty($name) && isset($this->need_names[$which])) {
                    echo("Template::Parse() : No NAME given to a ".$which." tag at ".$this->options['filename']." : line ".$lineNumber);
                    exit;
                }

                // Die if we got an escape but can't use one
                if ($escape and ($which != 'TMPL_VAR')) {
                    echo("Template::Parse() : ESCAPE option invalid in a ".$which." tag at ".$this->options['filename']." : line ".$lineNumber);
                    exit;
                }
                
                // Die if we got a default but can't use one
                if ($default and ($which != 'TMPL_VAR')) {
                    echo("Template::Parse() : DEFAULT option invalid in a ".$which." tag at ".$this->options['filename']." : line ".$lineNumber);
                    exit;
                }
                ...
                case 'TMPL_VAR':
                    ...
                    if (in_array($name, array('__pass__', '__passtotal__', '__counter__'))) {
                        if (count($inLoop)) {
                            ...
                            echo("Template::Parse() : Found context VAR tag outside of LOOP, at ".$this->options['filename']." : line ".$lineNumber);
                            exit;
                        }
                    } else {
                ...
                case '/TMPL_LOOP':
                    if (!strcmp(end($curType), "LOOP")) {
                        ...
                        echo("Template::Parse() : Nesting error: found end /TMPL_LOOP tag without its corresponding initial tag, at ".$this->options['filename']." : line ".$lineNumber." (last opened tag is of type \"".end($curType)."\")");
                        exit;
                    }
                    break;
                ...
                case 'TMPL_IF':
                    ...
                    if (in_array($name, array('__first__', '__odd__', '__inner__', '__last__'))) {
                        if (count($inLoop)) {
                            ...
                            echo("Template::Parse() : Found context IF/UNLESS tag outside of LOOP, at ".$this->options['filename']." : line ".$lineNumber);
                            exit;
                        }
                    } else {
                ...
                case '/TMPL_IF':
                    if (!strcmp(end($curType), "IF")) {
                        ...
                        echo("Template::Parse() : Nesting error: found end /TMPL_IF tag without its corresponding initial tag, at ".$this->options['filename']." : line ".$lineNumber);
                        exit;
                    }
                    break;
                ...
                case '/TMPL_UNLESS':
                    if (!strcmp(end($curType), "UNLESS")) {
                        ...
                        echo("Template::Parse() : Nesting error: found end /TMPL_UNLESS tag without its corresponding initial tag, at ".$this->options['filename']." : line ".$lineNumber);
                        exit;
                    }
                    break;
                ...
                case 'TMPL_ELSE':
                    if (!strcmp(end($curType), "IF") || !strcmp(end($curType), "UNLESS")) {
                        ...
                        echo("Template::Parse() : Nesting error: found end TMPL_ELSE tag without its corresponding initial tag, at ".$this->options['filename']." : line ".$lineNumber);
                        exit;
                    }
                    break;
                ...
                case 'TMPL_INCLUDE':
                    if (!$this->options['no_includes'])
                    {
                        if ($this->depth >= $this->options['max_includes'] && $this->options['max_includes'] > 0) {
                            echo("Template::Parse() : Include error: Too many included templates, found at ".$this->options['filename']." : line ".$lineNumber);
                            exit;
                        } else {
                            // BEGIN DEBUG
                            if ($this->options['debug']) {
                ...
                default:    
                    echo("Template::Parse() : Unknown or unmatched TMPL construct at ".$this->options['filename']." : line ".$lineNumber);
                    exit;
                    break;
                }
            } else {
                ...
                if ($this->options['strict'] && preg_match("/".$delim."\/?[Tt][Mm][Pp][Ll]_/", $chunks[$i])) {
                    echo("Template::Parse() : Syntax error in &lt;TMPL_*&gt; tag at " . $this->options['filename'] . " : line " . $lineNumber);
                    exit;
                }
        ...
        if (count($curType)) {
            echo("Template::Parse() : Template ".$this->options['filename']." incorrectly terminated. Found ".end($curType)." tag without corresponding end tag");
            exit;
        }
    }
        ...
        if (func_num_args() == 2) {
            if (is_scalar($value) || empty($value)) {
                ...
                if (isset($this->names[$arg])) {
                    ...
                    echo("Template::AddParam() : Attempt to set value for non existent variable name '".$arg."' - this variable name doesn't match any declarations in the template file");
                    exit;
                }
                ...
                if (isset($this->names[$arg])) {
                    ...
                    foreach($value as $a) {
                        if (!is_array($a)) {
                            // error
                            echo("Template::AddParam() : Must use arrays of associative arrays for loop parameters");
                            exit;
                        } else {
                ...
                // error
                echo("Template::AddParam() : Wrong value type");
                exit;
            }
            ...
            if (is_array($arg)) {
                ...
                // error
                echo("Template::AddParam() : Only one argument set, but it's not an array");
                exit;
            }
        } else {
            // error
            echo("Template::AddParam() : Wrong argument count (".func_num_args().") arguments");
            exit;
        }
    }
    ...
    function SetValue($arg, $value)
    {
        ...
        if (is_scalar($value) || empty($value)) {
            ...
            if (isset($this->names[$arg])) {
                ...
                echo("Template::SetValue() : Attempt to set value for non existent variable name '".$arg."' - this variable name doesn't match any declarations in the template file");
                exit;
            }
        } else{
            // error
            echo("Template::SetValue() : Value must be a scalar");
            exit;
        }
    }
        ...
        case "LOOP":
            if (isset($this->paramScope[count($this->paramScope)-1][$node->name])) {
                ...
                if (!is_array($this->paramScope[count($this->paramScope)-1][$node->name])) {
                    echo("Template->Output() : A scalar value was assigned to a LOOP var (".$node->name."), but LOOP vars only accept arrays of associative arrays as values,");
                    exit;
                }
                ...
                if ($lvl>=0) {
                    ...
                    if (!is_array($this->paramScope[$lvl][$node->name])) {
                        echo("Template->Output() : A LOOP var (".$node->name.") was trying to use a scalar value, but LOOP vars only accept arrays of associative arrays as values,");
                        exit;
                    }
    ...
    function SaveCompiled($outputdir = NULL, $overwrite = 0) {
        ...
        if (!is_writable($outputdir)) {
            echo("Template::SaveCompiled() - Output directory not writable");
            exit;
        } else {
            $file = basename($this->options['filename']);
            $output = $outputdir . "/" . $file . "c";
            if (file_exists($output) && !$overwrite) {
                echo("Template::SaveCompiled() - File " . $output . " already exists, cannot write compiled template");
                exit;
            } else {
                $f = fopen($output, "w");
...
class Node
{
    ...
    function Node($type, $name, $escape=NULL, $global=NULL,  $default=NULL)
    {
        $this->type    = $type;
        $this->name    = $name;
        $this->global  = $global;
...
function &LoadCompiledTemplate($filename) {
    if (!is_readable($filename)) {
        echo("LoadCompiledTemplate() - Cannot read file " . $filename);
        exit;
    } else {
        $f = fopen($filename, "r");
